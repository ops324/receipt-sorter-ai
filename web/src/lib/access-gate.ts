// ───────────────────────────────────────────────────────────
// パスワード（アクセスコード）ゲート。公開URLを「特定の顧客本人」だけに使わせ、
...
// なく PBKDF2-SHA256 + salt のハッシュを埋め込むため、バンドルを覗いてもパスワードは
// 読めない。部外者はまっさらなブラウザで来るため、期待値を localStorage 保存に
// すると新規ブラウザで素通りしてしまう → 必ずビルド側に持たせる。
//
// パスワードは 8 文字以上の推測しにくい語句を推奨（ハッシュがバンドルにあるため、
// 短い数字では総当たりされ得る）。
// ───────────────────────────────────────────────────────────

// ▼ ここにパスワードのハッシュを設定するとゲートが有効になる（空ならゲート無効=素通り）。
//   値は `node web/scripts/gen-access-code.mjs '<パスワード>'` で生成して貼り付ける。
...
// この環境でパスワードの検証ができるか（非セキュア接続では不可）。
export function canVerify(): boolean {
  return subtleAvailable();
}

// ゲートが有効か（＝パスワードハッシュが設定されているか）。未設定なら素通り（fail-open, opt-in）。
export function isGateEnabled(): boolean {
  return !!HASH_B64 && !!SALT_B64;
}

// 解錠済みフラグは「現在のハッシュの指紋」で保存する。パスワードを変更（ハッシュ差し替え）
