#!/usr/bin/env node
// ───────────────────────────────────────────────────────────
// 合言葉（アクセスコード）のハッシュ生成ツール。
//
// 使い方:
//   node web/scripts/gen-access-code.mjs '顧客に渡す合言葉'
//
// 出力された ACCESS_SALT_B64 / ACCESS_HASH_B64 を web/src/lib/access-gate.ts の
// 同名定数に貼り付けると、合言葉ゲートが有効になる。合言葉を変えたいときは再実行して
// 差し替えるだけ（既存端末は自動で再ゲートされる）。合言葉そのものはコミットしない。
//
...
if (!code || !code.trim()) {
  console.error("使い方: node web/scripts/gen-access-code.mjs '<合言葉>'");
  process.exit(1);
}
if (code.trim().length < 8) {
  console.error('⚠ 合言葉は8文字以上を推奨します（バンドルにハッシュが載るため）。');
}
