...
import { UploadCloud, Loader2, AlertCircle, CheckCircle2, Clock, ChevronRight, TrendingUp, KeyRound } from 'lucide-react';
...
export function Inbox({ onNavigate }: InboxProps) {
  ...
  function handleFiles(files: File[]) {
    ...
    if (files.length >= LARGE_IMPORT_THRESHOLD && settings?.hasApiKey) {
  ...
  async function runImport(files: File[]) {
    ...
    try {
      ...
      if (!settings?.hasApiKey) pushToast({ kind: 'info', message: 'APIキー未設定のため OCR は実行されませんでした。設定画面で登録してください。' });
      refreshCost();
    } finally {
      setBusy(false);
      setProgress(null);
    ...
    <div className="p-8 max-w-3xl mx-auto">
      ...
      {settings && !settings.hasApiKey && (
        <button onClick={() => onNavigate('settings')}
          className="mb-4 w-full flex items-center justify-between gap-2 px-4 py-3 bg-brand-50 border border-brand-200 rounded-xl text-sm text-brand-800 hover:bg-brand-100 transition-colors group">
          <span className="flex items-center gap-2.5">
            <KeyRound size={15} className="text-brand-600 shrink-0" />
            <span>OCRを使うには Anthropic APIキーの設定が必要です</span>
          </span>
          <span className="flex items-center gap-1 text-xs font-medium text-brand-700 shrink-0">
            設定を開く <ChevronRight size={13} className="group-hover:translate-x-0.5 transition-transform" />
          </span>
        </button>
      )}
